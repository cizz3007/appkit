...
type Compute<type> = { [key in keyof type]: type[key] }
...
// 개발 환경에서만 로깅을 위한 헬퍼 함수
function debugLog(message: string, ...args: unknown[]): void {
  if (process.env['NODE_ENV'] === 'development') {
    // eslint-disable-next-line no-console
    console.log(`[MSU Wallet] ${message}`, ...args)
  }
}
...
function debugError(message: string, ...args: unknown[]): void {
  if (process.env['NODE_ENV'] === 'development') {
    // eslint-disable-next-line no-console
    console.error(`[MSU Wallet] ${message}`, ...args)
  }
}

MsuWalletConnector.type = 'injected' as const

const connect_name = 'MSU Wallet'
const shimDisconnect = true
const unstable_shimAsyncInject = undefined

export function MsuWalletConnector() {
  function getTarget(): Compute<Target & { id: string }> {
    return {
      id: 'msuWallet',
      name: connect_name,
      provider: new WalletProviderApp(),
      icon: 'https://d1ghfhy4m942vn.cloudfront.net/logo/MSU_symbol.svg'
    }
  }
  ...
  return createConnector<Provider, Properties, StorageItem>(config => ({
    ...
    type: MsuWalletConnector.type,
    async setup() {
      debugLog('Setup 시작')
      const provider = await this.getProvider()
      debugLog('Provider 획득:', Boolean(provider))
    ...
    async connect({ chainId, isReconnecting } = {}) {
      debugLog('Connect 시작 - chainId:', chainId, 'isReconnecting:', isReconnecting)

      const provider = (await this.getProvider()) as WalletProviderApp
      if (!provider) {
        debugError('Provider를 찾을 수 없음')
        throw new ProviderNotFoundError()
      }
      debugLog('Provider 확인됨')

      let accounts: readonly Address[] = []
      if (isReconnecting) {
        debugLog('재연결 모드 - 기존 계정 확인 중')
        accounts = await this.getAccounts().catch(() => [])
        debugLog('재연결 계정 수:', accounts.length)
        ...
        try {
          ...
          accounts = (permissions[0]?.caveats?.[0]?.value as string[])?.map(x => getAddress(x))
          debugLog('권한 요청 성공, 계정 수:', accounts.length)
          if (accounts.length > 0) {
            const sortedAccounts = await this.getAccounts()
            accounts = sortedAccounts
            debugLog('정렬된 계정 수:', accounts.length)
          }
        } catch (err) {
          const error = err as RpcError
          debugLog('권한 요청 실패:', error.code, error.message)
      ...
      try {
        if (!accounts?.length && !isReconnecting) {
          debugLog('계정 요청 중...')
          // Msu wallet provider가 가지고 있는 iframe 실행
          const res = await provider.connect()
          console.log('응답 값: ', res)

          const account = res?.walletAddress
          const email = res?.email // 어떻게 쓸 지 나중에 생각

          accounts = [account as `0x${string}`]
          // const requestedAccounts = await provider.request({
          //   method: 'eth_requestAccounts'
          // })
          // debugLog('requestedAccounts', requestedAccounts)
          // accounts = requestedAccounts.map(x => getAddress(x))
          // debugLog('계정 요청 성공, 계정 수:', accounts.length)
        }

        debugLog('이벤트 리스너 설정 중...')
        if (connect) {
        ...
        debugLog('현재 체인 ID:', currentChainId, '요청된 체인 ID:', chainId)
        if (chainId && currentChainId !== chainId) {
          debugLog('체인 전환 시도 중...')

          const chain = await this.switchChain!({ chainId }).catch(error => {
            debugLog('체인 전환 실패:', error.code, error.message)
            if (error.code === UserRejectedRequestError.code) {
              throw error
            }
          ...
          debugLog('체인 전환 후 ID:', currentChainId)
        }

        // Remove disconnected shim if it exists
        if (shimDisconnect) {
          await config.storage?.removeItem(`${this.id}.disconnected`)
          debugLog('연결 해제 플래그 제거됨')
        }

        debugLog('연결 성공 - 계정:', accounts, '체인 ID:', currentChainId)

        return { accounts, chainId: currentChainId }
      } catch (err) {
        const error = err as RpcError
        debugError('연결 실패:', error.code, error.message)
      ...
      try {
        ...
        if (!provider) {
          if (unstable_shimAsyncInject !== undefined && unstable_shimAsyncInject !== false) {
            ...
            const timeout =
              typeof unstable_shimAsyncInject === 'number' ? unstable_shimAsyncInject : 1_000
...
/** @deprecated */
type TargetId =
  Compute<WalletProviderFlags> extends `is${infer name}`
